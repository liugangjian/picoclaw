...
func (sm *SensitiveDataMasker) AddDefaultRules() {
	...
	sm.AddRule(&MaskRule{
		...
		Replacement: `${1}://$2:***MASKED_DB_PASSWORD***@$3:$4$5$6`,
...
// SetCustomRules allows setting custom sensitivity rules
func (sm *SensitiveDataMasker) SetCustomRules(rules []*MaskRule) {
	sm.rules = rules // Replace with custom rules
}
...
// MaskBytes applies masking to byte slice
func (sm *SensitiveDataMasker) MaskBytes(data []byte) []byte {
	return []byte(sm.MaskString(string(data)))
}
...
// NewDefaultSensitiveDataMaskConfig returns a default configuration for sensitive data masking
func NewDefaultSensitiveDataMaskConfig() SensitiveDataMaskConfig {
    return SensitiveDataMaskConfig{
        Enabled: true,
        Rules: []SensitiveRuleConfig{
            {Name: "apiKey", Pattern: `(?i)(api[_-]?key|token|secret|password|pwd)\s*[=:]\s*['"]?([A-Za-z0-9_\-]{20,})['"]?`, Replacement: `***MASKED_API_KEY***`, Enabled: true},
            {Name: "openaiKey", Pattern: `sk-[A-Za-z0-9_-]{32,64}`, Replacement: `sk-***MASKED_OPENAI_KEY***`, Enabled: true},
            {Name: "anthropicKey", Pattern: `sk-ant-[A-Za-z0-9_-]{32,256}`, Replacement: `sk-ant-***MASKED_ANTHROPIC_KEY***`, Enabled: true},
        },
    }
}

func (sm *SensitiveDataMasker) MaskInterface(input interface{}) interface{} {
	switch val := input.(type) {
	case string:
		return sm.MaskString(val)
	case map[string]interface{}:
		return sm.MaskFields(val)
	case []interface{}:
		result := make([]interface{}, len(val))
		for i, v := range val {
			result[i] = sm.MaskInterface(v)
		}
		return result
	default:
		return input
